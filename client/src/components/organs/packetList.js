...
class PacketList extends Component {
    ...
    socket.on("packet", (data) => {
      ...
      if (data.payload.payload.decoderName == "ipv4") {
        ...
        newPackets.push({
          ...
          protocol: transportLayer.decoderName,
          length: length,
          buffer: buffer,
        });

        newPackets = newPackets.slice(
          Math.max(newPackets.length - maxPacketsAtOnce, 0)
        );
  ...
  getPacketsFor = (index) => {
    ...
    this.state.packets.forEach((packet, i) => {
      domPackets.push(
        <span
          onClick={() => this.props.inspectPacket(packet)}
          className={
            "hover:bg-gray-200 active:bg-gray-300 cursor-pointer flex " +
            packetColors[packet.protocol]
          }
          key={i}
        >
          <Resizable width={this.state.tdWidths[0]}>{packet.source}</Resizable>
          <Resizable width={this.state.tdWidths[1]}>
            {packet.destination}
          </Resizable>
          <Resizable width={this.state.tdWidths[2]}>
            {packet.protocol}
          </Resizable>
    ...
    return (
      <React.Fragment>
        <span className="text-gray-200 font-bold my-1">
          <Resizable i="0" setTdWidth={this.setTdWidth}>
            Source
          </Resizable>
          <Resizable i="1" setTdWidth={this.setTdWidth}>
            Destination
          </Resizable>
          <Resizable i="2" setTdWidth={this.setTdWidth}>
            Protocol
          </Resizable>
          <Resizable i="3" setTdWidth={this.setTdWidth}>
            Length
          </Resizable>
        </span>
        <div
          className="bg-gray-100 flex-1 flex-col overflow-auto "
          style={{ flex: "0 0 calc(100vh - 285px)" }}
        >
